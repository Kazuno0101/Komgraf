...
func draw_segienam(skala:float, rotasi:int, length:int, pos:Vector2, color:Color=Color.red, thick:int=1, dot:int=0, dash:bool=false):
	var point = Vector2(length, length)
	var a
	var b
	var temp
...
func draw_lingkaran(skala:float, pos:Vector2, r:int, color:Color):
	...
	circlePlotPoints(skala,xCenter, yCenter, x, y, color)
...
func circlePlotPoints(skala:float, xCenter:int, yCenter:int, x:int, y:int, color:Color, translate:Vector2 = Vector2(0,0)):	
	var point:Vector2
	
	point = Vector2(xCenter + x,  yCenter + y)
	point = trans_lingkaran(point,translate, skala, Vector2(xCenter,yCenter))	
	...
	point = Vector2(xCenter + y, yCenter + x)
	point = trans_lingkaran(point,translate, skala, Vector2(xCenter,yCenter))	
	put_pixel(point.x, point.y, color)
...
func trans_lingkaran(point:Vector2, translate:Vector2, skala:float, tikpus:Vector2):
	var temp = translasiVector2(point, translate.x, translate.y)
	temp = scaleVector2(temp, skala, tikpus)
